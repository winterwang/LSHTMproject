# This file is written to see the results in men and women 
...
# Day level graph in men, 2 day types, Time slots level 1 classes ----------------------------------------------
library(readr)
library(tidyverse)
...
cls1 <- ggplot(pp_long[pp_long$CW == 1, ], aes(y = Prob, x=Time, group = Carbo, 
  ...
  labs(title = "Class 1 days (48.6%)", x = "", 
       y = "Probability",
       color = "Carbohydrate\nintake") + 
...
cls2 <- ggplot(pp_long[pp_long$CW == 2, ], aes(y = Prob, x=Time, group = Carbo, 
  ...
  scale_linetype_manual(values=c("dotted", "dashed", "solid"),
                        labels =  c("No energy intake", "Carbohydrate < 50%", "Carbohydrate >= 50%")) + 
  # theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  labs(title = "Class 2 days (51.4%)", y = "Probability") + 
...
# Day level graph in men, 3 day types, Times slots level 1 classes --------

CW3CB2m <- read_table2("Rcode/MplusOutput/MultilevelMen/CW3CB2m.txt",
                      col_names = FALSE)

names(CW3CB2m) <- c("Breakfast", 
                   "Morning.snack",
                   "Lunch",
                   "Afternoon.snack",
                   "Dinner",
...
cls1 <- ggplot(pp_long[pp_long$CW == 3, ], aes(y = Prob, x=Time, group = Carbo, 
  ...
  scale_linetype_manual(values=c("dotted", "dashed", "solid")) + 
  # theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  labs(title = "Class 1 days - High percentage carbohydrate day 38.9%", x = "", 
       y = "Probability",
       color = "Carbohydrate\nintake") + 
  # scale_shape_discrete(labels = c("Not eating", "< 50%", ">= 50%")) + 
  ylim(c(0,1)) + 
...
cls2 <- ggplot(pp_long[pp_long$CW == 2, ], aes(y = Prob, x=Time, group = Carbo,
  ...
  scale_linetype_manual(values=c("dotted", "dashed", "solid")) + 
  # theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  labs(title = "Class 2 days - Low percentage carbohydrate day (29.4%)", x = "", 
       y = "Probability",
       color = "Carbohydrate\nintake") + 
  # scale_shape_discrete(labels = c("Not eating", "< 50%", ">= 50%")) + 
  ylim(c(0,1)) + 
...
cls3 <- ggplot(pp_long[pp_long$CW == 1, ], aes(y = Prob, x=Time, group = Carbo, 
  ...
  scale_linetype_manual(values=c("dotted", "dashed", "solid"),
                        labels =  c("No energy intake", "Carbohydrate < 50%", "Carbohydrate >= 50%")) + 
  # theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  labs(title = "Class 3 days - Regular meals days (38.2%)", y = "Probability") + 
  labs(shape="Responses to\ncarbohydrate intake", 
       linetype = "Responses to\ncarbohydrate intake")+
  # scale_shape_discrete(labels = c("Not eating", "< 50%", ">= 50%")) +
  ylim(c(0,1)) + 
...
# Day level graph in women, 2 day types, Time slots level 1 classes ----------------------------------------------
library(readr)
library(tidyverse)
