...
```{r echo=FALSE}
library(kableExtra)
dt <- read.csv("../Tablecsv/Modelfit.csv", header = T)
names(dt) <- c("N of classes", "N of parameters", "G^2", "df", "AIC", "BIC", "log-likelihood")

kable(dt, digits = 3, row.names = FALSE, align = "c",
              format = "html", caption = "Model Comparison.") %>%
 kable_styling(font_size = 16, 
        bootstrap_options = c("striped", "hover", "condensed"),
        position = "center", full_width = FALSE) %>% 
  row_spec(5, bold = T, color = "white", background = "#D7261E") %>% 
  footnote(general = "Abbreviation: N, number; G^2, the likelihood ratio statistic; AIC, Akaike information criterion; BIC, Bayesian information criterion.")
```
